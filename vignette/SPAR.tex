...
\item
  ...
  \citet{Clarkson2013LowRankApprox} for \texttt{rp\_cw(data\ =\ FALSE)}.
...
\item
  ...
  \texttt{x} -- the matrix of standardized predictors, \texttt{y} -- the
  vector of (standardized in the Gaussian case) responses and
  \texttt{screencoef} -- a \class{screencoef} object relevant for case
  where the screening coefficient should be reused in constructing the
  projection matrix. Returns a \class{randomprojection} object.
\item
...
R> spar_res
#> spar object:
#> Smallest Validation Measure of 3.18e+04 reached for nummod=30,
#>               nu=1.72e-02 leading to 1002 / 2000 active predictors.
#>  spar object:
#> Smallest Validation Measure of 3.11e+04 reached for nummod=30,
#>               nu=1.72e-02 leading to 1002 / 2000 active predictors.
#>  spar object:
#> Smallest Validation Measure of 3.16e+04 reached for nummod=30,
#>               nu=1.72e-02 leading to 1002 / 2000 active predictors.
#>  spar object:
#> Smallest Validation Measure of 3.10e+04 reached for nummod=30,
#>               nu=1.72e-02 leading to 1002 / 2000 active predictors.
#>  spar object:
#> Smallest Validation Measure of 3.08e+04 reached for nummod=30,
#>               nu=1.72e-02 leading to 1002 / 2000 active predictors.
#>  spar object:
#> Smallest Validation Measure of 3.05e+04 reached for nummod=30,
#>               nu=1.72e-02 leading to 1002 / 2000 active predictors.
...
+    parallel = TRUE)
#> Using doParallelSNOW with 2 workers
...
R> spar_example
#> spar object:
#> Smallest Validation Measure of 1.32e+02 reached for nummod=20,
...
\begin{verbatim}
R> update_data_cannings <- function(rp, x, y, screencoef = NULL) {
...
We start by defining the \texttt{model\_fun()} function:

\begin{verbatim}
R> model_glmrob <- function(y, z, object) {
+  ## y - vector of n responses
+  ## z - matrix with n rows
+  glmrob_res <- do.call(function(...) robustbase::glmrob(y ~ z, ...),
+                       object$control)
...
\begin{verbatim}
R> spar_glmrob <- constructor_sparmodel(name = "glmrob", model_fun = model_glmrob)
\end{verbatim}

We can now the SPAR algorithm with robust GLMs are marginal models and
compare it to a version with marginal GLMs. For illustration purposes we
contaminate 10\% of the \texttt{example\_data} with outliers both in the
predictors and response variable. We find the threshold \(\nu\) by
choosing the value which delivers the lowest mean absolute error.
...
R> y <- example_data$y
R> set.seed(123)
R> id_outliers_x <- sample(seq_len(ncol(x)*nrow(x)), 
+                      tau * ncol(x)*nrow(x))
R> x[id_outliers_x] <- x[id_outliers_x] + 20
R> id_outliers_y <- sample(seq_len(length(y)), tau * length(y))
R> y[id_outliers_y] <- y[id_outliers_y] - 20
R> set.seed(123)
R> spar_rob_res <- spar(x, y,
+                     model = spar_glmrob(),
+                     rp = rp_gaussian(),
...
R> spar_rob_res
#> spar object:
#> Smallest Validation Measure of 1.15e+01 reached for nummod=50,
#>               nu=1.99e-03 leading to 1919 / 2000 active predictors.
#> Summary of those non-zero coefficients:
#>      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
#> -0.046434 -0.001595  0.005086  0.008388  0.015549  0.109824
R> spar_res
#> spar object:
#> Smallest Validation Measure of 1.15e+01 reached for nummod=50,
#>               nu=1.99e-03 leading to 1919 / 2000 active predictors.
...
R> spar_faces
#> spar.cv object:
#> Smallest CV-Meas 12.6 reached for nummod=50,
#>               nu=0.00e+00 leading to 3689 / 4096 active predictors.
#> Summary of those non-zero coefficients:
#>      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
#> -652.8332   -0.0822    0.0000    0.6897    0.0945 1291.5781 
#> 
#> Sparsest coefficient within one standard error of best CV-Meas
#>               reached for nummod=5, nu=3.45e-03 
#> leading to 1597 / 4096 active
#>               predictors with CV-Meas 17.1.
#> Summary of those non-zero coefficients:
#>      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
