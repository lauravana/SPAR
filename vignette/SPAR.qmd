...
well in terms of prediction power on a variety of data sets. @Dunson2020TargRandProj employ the algorithm with the sparse random projection 
of @ACHLIOPTAS2003JL combined with
probabilistic marginal correlation screening. 
Furthermore, @parzer2024glms show that when paired with a carefully constructed
...
-   `rp` an object of class [randomprojection]{.class}. Defaults 
to `NULL`, in which case `rp_cw(data = TRUE)` is used.

-   `screencoef` an object of class [screencoef]{.class},
Defaults to `NULL`, in which case no screening is employed.
...
- `parallel` assuming a parallel backend is loaded and available, a logical indicating whether the function should use it. Defaults to `FALSE`.
...
<!-- We now delve deeper into the structure of these functions. -->
<!-- Consider as an example function `screen_marglik()`: -->
<!-- ```{r} -->
<!-- screen_marglik -->
<!-- ``` -->
...
be passed to the `screen_*()` function through `...`, and will be saved as attributes of the [screencoef]{.class} object.
...
the SPAR algorithm and are present in all [randomprojection]{.class}
objects:
...
* `rp_sparse()` -- random projection object where the generated matrix will be the one in [@ACHLIOPTAS2003JL]. The value of $\psi$ can be passed through the `...` argument and defaults to  `psi = 1`.

* `rp_cw()` -- sparse embedding random projection in @Clarkson2013LowRankApprox. If specified as `rp_cw(data = TRUE)`, 
 the 
random elements on the diagonal are replaced by the ridge coefficients with a small penalty, as introduced in @parzer2024glms.
...
For the case where the random projection should incorporate some information related to the data, two more elements can be 
potentially relevant. 

* Function `update_fun()` updates the [randomprojection]{.class}
object with relevant information from the arguments of the `spar()`
function call. If it is not provided, it defaults to updating the [randomprojection]{.class} object with a further attribute 
`family_string` which is character version of the `family` argument 
(e.g., in the default case it will be `"gaussian(identity)"`).
In certain constructions, such as the one
in @parzer2024glms, certain data dependent quantities (in the case of 
@parzer2024glms these would be the ridge coefficients estimated on the whole data set) need to be computed only once and not every time 
a random projection is generated. Function `update_fun()` can also be
employed to add data information
as attributes of  the [randomprojection]{.class} object to be 
subsequently used in the `generate_fun()` function.
If specified by the user, this function should take as argument only \code{...}. Internally in the `spar()` function, all
arguments of `spar()` are passed to this function. It should returns a [randomprojection]{.class} object.

* In the case where a list of predefined `RPMs` is provided in `spar()`,
for the data driven random projections we allow the user to 
specify whether some parts of the given `RPMs` should be updated with
the provided data. This is possible through another optional 
function  `update_rpm_w_data()`. 
This is particularly relevant for the cross-validation procedure,
which employs the random projection matrices 
generated by calling the `spar()` function on the whole data set
before starting the cross-validation exercise. For example, in our
implementation of the data-driven `rp_cw()`,
in each fold, we only update the list of `RPMs` by adjusting the
diagonal elements to the vector of screening coefficients computed on
the current training data for the current fold, but do not modify the
random elements in each fold, to reduce the computational burden.
Defaults  to `NULL`. If not provided, the values of the provided
`RPMs` do not change. 
...
* `update_fun()` -- an optional function which can add further attributes to 
...
We estimate the SPAR algorithm with no screening and 
`rp_cw(data = TRUE)` (default).
```{r first_example}
set.seed(12)
...
marginal models in the ensemble via package [foreach]{.pkg} [@pkg:foreach].
This is possible by setting the parameter `parallel` to `TRUE`,
assuming that a parallel backend for [foreach]{.pkg} is registered 
using the `registerDoParallel()` function of package [doParallel]{.pkg} [@pkg:doParallel]. A minimal example with a correlation-based 
probabilistic screening and a Gaussian random projection matrix
is provided below:
```{r parallel_example,message=FALSE}
library(doParallel)
...
The employed random projection is `rp_sparse()` where we 
set $\psi=1/\sqrt{p}$, as proposed by @LiHastie2006VerySparseRP.
```{r}
set.seed(123)      
...
We can start implementing such a random projection in [spar]{.pkg}
by the following steps. First, there are no data quantities which are to be used 
in all possible random projections so we do not need to specify a function `update_fun()`. However, we can use `update_fun()`
to update the [randomprojection]{.class} object with further 
information related to the family. Given that we will rely on `glmnet::glmnet()` in each loop, the family object is modified to a string containing the name of the family for families Gaussian, binomial and  Poisson with  canonical link (this will lead to using faster specialized algorithms). This modified version is 
added to the list of controls as `fit_family`.
We also by set a default of
$\alpha = 0$ in the [glmnet]{.pkg} model to be estimated inside the function generating  the random projection, $B_2=50$ and $\xi=0.25$.
```{r}
...
```
As we can see, values for `xi` and `B2` can be passed by the user through the  `control` argument.
...
Finally, we illustrate how to implement a new marginal model in [spar]{.pkg}.
We consider estimating a robust GLM as a marginal model using the 
package [robustbase]{.pkg} [@pkg:robustbase].

We start by defining the `model_fun()` function, 
where  `y` is the vector of responses, `z` is the matrix of reduced predictors and `object` is a [sparmodel]{.class} object. In the case of a linear model, we rely on the function
`robustbase::lmrob()`, for other family-links we use 
`robustbase::glmrob()`.
```{r}
...
```
We then construct a `spar_glmrob()` function, which builds the [sparmodel]{.class}
...
contaminate $\tau=25$% of the `example_data` with outliers in the predictors and consider a binary version of the response variable `y`.
```{r contaminate_x_y}
tau <- 0.25
x <- example_data$x;
set.seed(123)
np <- ncol(x)*nrow(x)
id_outliers_x <- sample(seq_len(np), tau * np)
...
`rp_gaussian()`. We set `measure = "mae"` i.e., we find the threshold $\nu$ by choosing the value
