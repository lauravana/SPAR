...
* Function `update_rp_fun()` updates the [randomprojection]{.class}
...
a random projection is generated. Function `update_rp_fun()` can also be
...
If specified by the user, this function should take as argument
only \code{...}. Internally in the `spar()` function, all
...
using the `registerDoParallel()` function of package [doParallel]{.pkg} [@pkg:doParallel]. A minimal example is provided below:
...
and for each model $k$, $B_2$ data independent
...
then estimates a ridge regression on the training data and compute the misclassification error
for the binomial family and MSE for all other families on the test set.
Finally the best out of $B2$ projections in terms of minimizing the loss on the test set is chosen.
...
in all possible random projections so we do not need to specify a function `update_rp_fun()`. However, we can use `update_rp_fun()`
...
generate_cannings <- function(rp, m, included_vector, x, y) {
  ...
  for (b in seq_len(B2)) {
    ...
    mod <- do.call(function(...) 
      glmnet::glmnet(x = xrp, y = ytrain, family = rp$control$fit_family, ...),
      control_glmnet)
    ...
    rm(RM)
   # print(tracemem(best_RM))
  }
  return(best_RM)
}
```
As we can see, values for `xi` and `B2` can be passed by the user through the 
`control` argument.
...
rp_cannings <- constructor_randomprojection(
  ...
  update_rp_fun = update_rp_cannings
)
```
...
We can now the SPAR algorithm with robust GLMs are marginal models and compare 
it to a version with marginal GLMs. For illustration purposes we 
contaminate 10% of the `example_data` with outliers both in the predictors
and response variable. We find the threshold $\nu$ by choosing the value
...
y <- example_data$y
set.seed(123)
id_outliers_x <- sample(seq_len(ncol(x)*nrow(x)), 
                        tau * ncol(x)*nrow(x))
x[id_outliers_x] <- x[id_outliers_x] + 20
id_outliers_y <- sample(seq_len(length(y)), tau * length(y))
y[id_outliers_y] <- y[id_outliers_y] - 20
set.seed(123)
spar_rob_res <- spar(x, y,
                     model = spar_glmrob(),
                     rp = rp_gaussian(),
